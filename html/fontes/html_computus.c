/* html_computus.c - imago memoriae et temporis parseris HTML (.h) */
#include "latina.h"
#include "piscina.h"
#include "chorda.h"
           ...
           HtmlComputus* exitus)
{
    ...
    piscina = piscina_generare_dynamicum("html_computus_parsare",
        HTML_COMPUTUS_ALVEUS_INITIUM);
    si (piscina == NIHIL)
    {
        redde FALSUM;
    }
    ...
    arbor_idem   = FALSUM;
    {
        ...
        altera = piscina_generare_dynamicum("html_computus_arbor",
            HTML_COMPUTUS_ALVEUS_INITIUM);
        si (altera == NIHIL)
        {
            piscina_destruere(piscina);
        ...
        si (materia_lexicon_ratum_facere(&ratum, &HTML_LEXICON,
            &iudicium))
        {
