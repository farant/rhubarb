...
# Effusio cursus ultimi semper in build/test_logs/html.log
# radicis (stdout + stderr; codex exitus per PIPESTATUS servatus).
if [ -z "${HTML_PROBATIONES_EFFUSIO:-}" ]; then
    export HTML_PROBATIONES_EFFUSIO=1
    mkdir -p "$(dirname "$0")/../build/test_logs"
    "$0" "$@" 2>&1 | tee "$(dirname "$0")/../build/test_logs/html.log"
    exit "${PIPESTATUS[0]}"
fi

# html/compile_probationes.sh - Probationes clientis html (materia)
#
# Usage:
...
MATERIA_DIR="$(cd "$HTML_DIR/../materia" && pwd)"
...
done < <(find "$RADIX_DIR/include" "$HTML_DIR/fontes" "$HTML_DIR/probationes" \
             -name "*.h" 2>/dev/null)
...
shopt -s nullglob
# materia sub-fontes: html eam CONSUMIT, non continet
for m in materia_lexicon materia_token materia_nodus materia_scribere \
         materia_arbor materia_arbor_aequalitas materia_coctor; do
...
for src in "$HTML_DIR"/fontes/*.c; do
    base="$(basename "$src" .c)"
    obj="$BUILD_DIR/$base.o"
    if [ ! -f "$obj" ] || ! [ "$obj" -nt "$src" ] || [ -n "$(newest_header "$obj")" ]; then
        echo "  [html] $base.c"
...
# "html." - silva.mensurae('html.', n) eas legit); numquam suitam frangit
source "$RADIX_DIR/tools/mensor_suitae.sh"
mensor_suitae_incipere "html."
total=0 ; passed=0 ; failed_names=""
for test_file in "$HTML_DIR"/probationes/probatio_*.c; do
...
echo "========================================"
echo "HTML PROBATIONES: $passed/$total praeteritae"
