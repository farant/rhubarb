...
hic_manens constans character* CORPUS[NUMERUS_CORPORIS] = {
    "adversarius.html",
    "adversarius_2.html",
    "laboratorium_index_2026-08-18.html",
    "villa_index_2026-08-18.html"
};
...
principale (vacuum)
{
    ...
    piscina = piscina_generare_dynamicum("probatio_html_computus",
        MMMMXCVI * XVI);
    si (!piscina)
    {
    ...
    sprintf(via_auri, "%s/html/probationes/fixa/computus/basis.tsv",
        radix);

    imprimere("\n--- I. mensura corporis (fixa/html) ---\n");
    per (i = ZEPHYRUM; i < NUMERUS_CORPORIS; i++)
    {
        ...
        metita[i] = FALSUM;
        sprintf(via, "%s/probationes/fixa/html/%s", radix, CORPUS[i]);
        ...
        (vacuum)html_computus_metiri((constans character*)fons, mensura,
            &c);
        _columnas(&c, facta[i]);
        metita[i] = VERUM;
