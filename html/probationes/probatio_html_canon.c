/* probatio_html_canon.c - Porta canonis HTML (H7)
 *
 * html/grammatica/html.canon MANU SCRIPTUM est (exemplar css B7,
 * decretum Frani 2026-09-01: registrum ex declaratione generatur,
 * canon manu). Canon manu scriptus SILENTER
 ...
 *      poscit).
 *
 * IUDICIUM: documentum quodque quod porta H6 parit (corpus XIV
...
principale (vacuum)
{
    ...
    piscina = piscina_generare_dynamicum("probatio_html_canon",
    ...
    per (i = ZEPHYRUM; i < HTML_REGISTRUM.numerus_generum; i++)
    {
        constans character* titulus;
                       i32  quot;

        titulus = HTML_REGISTRUM.genera[i].titulus;
    ...
    per (i = ZEPHYRUM; i < HTML_REGISTRUM.numerus_generum; i++)
    {
        constans MateriaTabGenus* genus = &HTML_REGISTRUM.genera[i];
                             i32  j;

        per (j = ZEPHYRUM; j < genus->loci_numerus; j++)
        {
            constans character* locus =
                HTML_REGISTRUM.loci[genus->loci_offset + j].titulus;
    ...
    per (i = ZEPHYRUM; i < HTML_LEXICON.numerus_generum; i++)
    {
        character tag[LXIV];
              i32 quot;

        _tag_lexematis(tag, HTML_LEXICON.praefixum_tagi,
            HTML_LEXICON.genera[i].titulus);
    ...
    imprimere("\n--- Iudicans corpus ---\n");
    {
        ...
        CREDO_AEQUALIS_I32 (lectae, XIV);
    }

    /* se metiens */
    CREDO_MAIOR_I32 (documenta_iudicata, XIV);
