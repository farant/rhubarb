...
## 2026-09-22 — audit, step VI: string bytes must be valid UTF-8

Found by the closing edge probe: bytes inside strings were never
checked. `FF FE`, an overlong `C0 AF`, a UTF-8-encoded surrogate
`ED A0 80` all parsed, and the writer passed them straight back out —
output other JSON parsers reject (RFC 8259 requires UTF-8).

The string lexer now hands every byte >= 0x80 to `utf8_decodere`
(include/utf8.h, already house-wide). It is strict — overlong forms,
encoded surrogates, > U+10FFFF, truncated sequences, orphan
continuation bytes and F8+ lead bytes all return -1 — so the lexer
only has to refuse on -1 ("UTF-8 invalidum in chorda") at the lead
byte, or step over the validated sequence. Keys go through the same
path. While stepping, the column now advances ONCE per character
(`columna -= n - 1` after n byte-advances): error columns count
characters, not bytes (was: é = 2 columns).

NEW LINK DEPENDENCY json.c → utf8.c. Every generated source list that
contains json was regenerated by the command in its own header (six
gained "utf8": gesta ×4 incl. both residents, officina legatus/sonda;
the regeneration also swept closure drift — `fasti` into legatus and
sonda, `processus_posix` into sonda). Two HAND arrays name json and
got "utf8" by hand: briar/compile_probationes.sh,
officina/compile_probationes.sh. (The frigida launcher sources the
tabularium list.)

Live data: 466 annal lines carry raw UTF-8 (4194 high bytes; forum 119
lines, tabularium 347) — all pass. The writer still does NOT validate:
a builder can put arbitrary bytes into a chorda and they are emitted as
given (documented in json.h; the writer has no error channel).

Tests `probatio_utf8`: five valid forms (2/3/4 bytes, U+10FFFF, U+FFFD
