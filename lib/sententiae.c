    ...
    chorda* linea)
{
    ...
    si (totum.datum == NIHIL || *positus >= totum.mensura)
    { redde FALSUM;
    }
    ...
    chorda s)
{
    ...
    dum (a < b && (_album(s.datum[a]) || s.datum[a] == (i8)'*'))
    { a++;
    }
       ...
       i32* profunditas)
{
    ...
    si (linea.datum == NIHIL || linea.mensura == ZEPHYRUM)
    { redde FALSUM;
    }
    ...
    dum (VERUM)
    {
        ...
        {
            dum (i < linea.mensura && _numeralis(linea.datum[i]))
            { i++;
            }
        }
        ...
        alioquin
        {
            dum (i < linea.mensura && _minuscula(linea.datum[i]))
            { i++;
            }
        }
        ...
        i++;
        segmenta++;
        si (segmenta > (i32)SENTENTIAE_PROFUNDITAS_MAXIMA)
        { redde FALSUM;
        }
          ...
          chorda* onus)
{
    ...
    per (k = ZEPHYRUM; k < xar_numerus(vocabularium); k++)
    {
        ...
        si (chorda_aequalis(g->vocabulum, contentum))
        { redde (s32)k;
        }
    ...
    Piscina* piscina)
{
    ...
    dum (i < prooemium.mensura)
    {
        ...
        j = ZEPHYRUM;
        dum (j < contentum.mensura && _minuscula(contentum.datum[j]))
        { j++;
        }
        si (j == ZEPHYRUM)
        { i = clausum + I; perge;
    ...
    chorda fons)
{
    ...
    dum (VERUM)
    {
        ...
        si (!_linea_proxima(fons, &positus, &linea))
        { frange;
        }
    ...
    chorda c)
{
    ...
    {
        si (c.datum[i] == (i8)'-' || c.datum[i] == (i8)':')
        { signa++;
        }
        alioquin si (!_album(c.datum[i]))
        { redde FALSUM;
        }
    }
    ...
    Piscina* piscina)
{
    ...
    dum (_linea_proxima(prooemium, &positus, &linea))
    {
        ...
        si (!chorda_incipit(chorda_praecidere(linea), _ch("|")))
        { perge;
        }

        fissio = chorda_fissio(linea, '|', piscina);
        si (fissio.numerus < (i32)IV)
        { perge;
        }
        ...
        si (siglum.mensura == ZEPHYRUM)
        { perge;
        }
        si (_cella_separatrix(siglum))
        { perge;
        }
        si (chorda_aequalis_literis(siglum, "Siglum"))
        { perge;
        }
             ...
             Piscina* piscina)
{
    ...
    dum (i < textus.mensura)
    {
        ...
        { redde SENTENTIAE_UNCINUS_APERTUS;
        }
...
_sigla_extrahere (
                   chorda  textus,
             constans Xar* auctoritates,
                Sententia* s,
                  Piscina* piscina)
{
    i32 k;

    si (auctoritates == NIHIL)
    { redde;
    }
    ...
    per (k = ZEPHYRUM; k < xar_numerus(auctoritates); k++)
    {
        ...
        si (a == NIHIL || a->siglum.mensura == ZEPHYRUM)
        { perge;
        }
        ...
        per (i = ZEPHYRUM; i + a->siglum.mensura <= textus.mensura; i++)
        {
            ...
            si (!chorda_aequalis(candidatus, a->siglum))
            { perge;
            }
            ...
            chorda  locus)
{
    ...
    si (liber == NIHIL || liber->sententiae == NIHIL)
    { redde NIHIL;
    }
    ...
    per (k = ZEPHYRUM; k < xar_numerus(liber->sententiae); k++)
    {
        ...
        si (s != NIHIL && chorda_aequalis(s->locus, locus))
        { redde s;
        }
    }
    redde NIHIL;
    ...
    Piscina* piscina)
{
    ...
    per (k = ZEPHYRUM; k < xar_numerus(liber.sententiae); k++)
    {
        ...
        si (s == NIHIL || s->remissiones == NIHIL)
        { perge;
        }
        ...
        per (j = ZEPHYRUM; j < xar_numerus(s->remissiones); j++)
        {
            ...
            si (r == NIHIL || r->genus != REMISSIO_INTERNA)
            { perge;
            }
