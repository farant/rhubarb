...
(function () {
    ...
    /* ================= titulus ================= */
    var el5 = { 'theca-1': { titulus: 'recepta', tabula: 'radix' } };
    proba(mensaTitulus(el5, 'radix') === null,
        "titulus: radix sine titulo");
    proba(mensaTitulus(el5, 'theca-1') === 'recepta',
        "titulus: e theca nominante");
    proba(mensaTitulus({}, 'theca-x') === 'theca-x',
        "titulus: orphana -> id ipsum");
    proba(mensaTitulus({ 'theca-2': { x: 1 } }, 'theca-2')
        === 'theca-2', "titulus: theca sine titulo -> id");

    /* ================= ordo (levatio) ================= */
    proba(mensaOrdoSummus({}) === 0, "ordo: vacuum -> 0");
    proba(mensaOrdoSummus({ a: { x: 1 }, b: { y: 2 } }) === 0,
        "ordo: sine ordinibus -> 0");
    var el6 = { a: { ordo: 2 }, b: { ordo: 5 }, c: { x: 1 } };
    proba(mensaOrdoSummus(el6) === 5, "ordo: summus inventus");

    proba(mensaLevatio(el6, 'a') === 6, "levatio: infra -> summus+1");
    proba(mensaLevatio(el6, 'b') === null,
        "levatio: sola summa -> null (acta quieta)");
    proba(mensaLevatio(el6, 'c') === 6, "levatio: sine ordine -> summus+1");
    proba(mensaLevatio({}, 'ignota') === 1, "levatio: statu vacuo -> 1");
    var el7 = { a: { ordo: 3 }, b: { ordo: 3 } };
    proba(mensaLevatio(el7, 'a') === 4,
        "levatio: aequalitas in summo levationem meret");
    /* levatum per plicam fluit ut genus quodlibet */
    var el8 = { a: { x: 1 } };
    mensaFundere(el8, 'levatum', { id: 'a', ordo: 7 });
    proba(el8.a.ordo === 7 && el8.a.x === 1,
        "fundere: levatum ordinem fundit, claves aliae manent");

    /* ================= paginator ================= */
    var pag = new MensaPaginator(3);
    var ordo9 = ['a','b','c','d','e','f','g','h','i'];
    proba(pag.paginae(9) === 3, "paginator: paginae computatae");
    proba(pag.paginae(0) === 1, "paginator: vacuum -> pagina una");
    proba(pag.fenestra(ordo9).join('') === 'abc',
        "paginator: fenestra prima");
    proba(pag.post(9) === true && pag.fenestra(ordo9).join('') === 'def',
        "paginator: post movet");
    proba(pag.ante() === true && pag.pagina === 0,
        "paginator: ante revenit");
    proba(pag.ante() === false, "paginator: ante in prima clausum");
    pag.post(9); pag.post(9);
    proba(pag.post(9) === false, "paginator: post in ultima clausum");
    proba(pag.fenestra(['x','y']).join('') === 'xy',
        "paginator: ordo brevior paginam artat");
    proba(pag.pagina === 0, "paginator: artatio ad exsistentem");
    proba(pag.statum(9) === '1/3', "paginator: statum forma n/m");
    proba(new MensaPaginator(0).quantum === 10,
        "paginator: quantum invalidum -> ordinarius");

    /* ================= partitio linearum ================= */
    proba(mensaOrdinesLineae(0, 40) === 1, "ordines: vacua = 1");
    proba(mensaOrdinesLineae(40, 40) === 1, "ordines: exacta = 1");
    proba(mensaOrdinesLineae(41, 40) === 2, "ordines: involuta = 2");
    proba(mensaOrdinesLineae(10, 0) === 10, "ordines: columnae artatae");

    var initia = mensaPartitioLinearum([1,1,1,1,1], 2);
    proba(initia.join(',') === '0,2,4', "partitio: aequalis");
    initia = mensaPartitioLinearum([2,1,2,1], 3);
    proba(initia.join(',') === '0,2', "partitio: cumulativa");
    initia = mensaPartitioLinearum([5], 3);
    proba(initia.join(',') === '0',
        "partitio: linea maior paginam propriam tenet");
    initia = mensaPartitioLinearum([1,5,1], 3);
    proba(initia.join(',') === '0,1,2',
        "partitio: maior in medio non trahit vicinos");
    proba(mensaPartitioLinearum([], 3).join(',') === '0',
        "partitio: vacua = pagina una");

    initia = [0, 4, 9];
    proba(mensaPaginaAncorae(initia, 0) === 0, "ancora: prima");
    proba(mensaPaginaAncorae(initia, 3) === 0, "ancora: intra primam");
    proba(mensaPaginaAncorae(initia, 4) === 1, "ancora: in limine");
    proba(mensaPaginaAncorae(initia, 7) === 1, "ancora: media pagina");
    proba(mensaPaginaAncorae(initia, 99) === 2, "ancora: ultra finem");
