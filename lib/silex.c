    ...
    constans character* titulus)
{
    ...
    chorda_aedificator_appendere_literis(a,
        ...
        "    }\n"
        "\n"
        "    /* -volumen <via>: STATUM nominare. Sine hoc applicatio"
        " SEMPER\n"
        "     * ~/.rhubarb scribit, et probatio integrationis quaeque"
        " in\n"
        "     * volumen VERUM usoris scribit - acta accumulantur,"
        " numerus\n"
        "     * visitationum omni cursu crescit, et probatio"
        " repetibilis\n"
        "     * esse desinit. Vexillum hoc idem est quod agitator"
        " (manus)\n"
        "     * poscit: probatio identitatem applicationis regere"
        " debet,\n"
        "     * et status identitas est non minus quam portus.\n"
        "     * ISOLATIO OPUS APPLICATIONIS EST, non agitatoris. */\n"
        "    via_petita = NIHIL;\n"
        "    per (i = I; i < argc - I; i++)\n"
        "    {\n"
        "        si (strcmp(argv[i], \"-volumen\") == ZEPHYRUM)\n"
        "        {\n"
        "            via_petita = argv[i + I];\n"
        "        }\n"
        "    }\n"
    ...
    chorda_aedificator_appendere_literis(a,
        ...
        "    }\n"
        "\n"
        "    /* VOLUMEN POST ATRIUM de industria: volumen aperire"
        " plagulam\n"
        "     * CREAT. Ante atrium apertum, cursus qui vexillo pravo"
        " statim\n"
        "     * moritur nihilominus statum in disco relinquebat"
        " (mensuratum\n"
        "     * 2026-08-14 in silicetum/003). Validatio vilis effectus"
        "\n"
        "     * praecedat. */\n"
        "    via = (via_petita != NIHIL)\n"
        "        ? via_petita\n"
    ...
    constans character* titulus)
{
    ...
    chorda_aedificator_appendere_literis(a,
        ...
        "    credo_aperire(piscina);\n"
        "\n"
        "    /* Volumen TEMPORARIUM: sub /tmp nascitur, pid fert (ergo"
        " duo\n"
        "     * cursus non concurrunt), et CLAUDENDO se delet. Ergo"
        " nulla\n"
        "     * remove() manualis, nullum vestigium, et cursus quisque"
        " a\n"
        "     * statu NOTO incipit - quod probationem repetibilem"
        " facit. */\n"
        "    volumen = volumen_temporarium(piscina, \"probatio_");
    chorda_aedificator_appendere_literis(a, titulus);
    chorda_aedificator_appendere_literis(a,
        "\");\n"
        ...
        "    }\n"
        "\n"
        "    /* claudere = delere. Qui omittit plagulam in /tmp"
        " relinquit,\n"
        "     * quod ipsum indicium est. */\n"
    ...
    constans character* titulus)
{
    ...
    chorda_aedificator_appendere_literis(a, titulus);
    chorda_aedificator_appendere_literis(a,
        " -radix assets` — assetum e DISCO, non e capsula\n"
        "  infixa: muta `assets/`, recarica, vide — sine"
        " aedificatione.\n"
        "  Schema et URL manent eadem, ergo pagina discernere non"
        " potest.\n"
        "  Via RELATIVA cwd est: e RADICE proiecti currendum (aliter"
        "\n"
        "  recusat, nominatim). Independens a `-vivum`.\n"
        "- `./bin/");
    chorda_aedificator_appendere_literis(a, titulus);
    chorda_aedificator_appendere_literis(a,
        " -volumen /tmp/x.volumen` — STATUM alibi ponere.\n"
        "  Sine hoc app semper `~/.rhubarb/");
    chorda_aedificator_appendere_literis(a, titulus);
    chorda_aedificator_appendere_literis(a,
        ".volumen` scribit —\n"
        "  quod probationem integrationis in datis VERIS tuis"
        " currere\n"
        "  cogeret. Isolatio opus APPLICATIONIS est.\n"
        "- `./probare.sh` — probationes (volumen temporarium sub"
        " /tmp,\n"
        "  claudendo deletum: nullum vestigium, cursus repetibiles)\n"
    ...
    constans SilexNovumOptiones* optiones)
{
    ...
    si (!optiones->vitrea)
    {
            ...
            _probationem_fingere(piscina, optiones->titulus));
        {
            ...
            fontes_app[0]  = _texere(piscina, "fontes/",
                optiones->titulus, ".c");
    ...
    constans character* fontes_prob[2];
        ...
        res_omnes = silex_clausuram_e_contentis(piscina,
        ...
        clausura_probationis = silex_clausuram_e_contentis(piscina,
        ...
        clausura_fumi = silex_clausuram_e_contentis(piscina,
        ...
        fontes_app[0]  = _texere(piscina, "fontes/", optiones->titulus,
            ".c");
        fontes_app[1]  = _texere(piscina, "fontes/", optiones->titulus,
            "_pipa.c");
        fontes_prob[0] = _texere(piscina, "probationes/probatio_",
            optiones->titulus, ".c");
        fontes_prob[1] = fontes_app[1];
        ordo_aedificandi = silex_ordinem_vitreum_fingere(piscina,
            optiones->titulus, fontes_app, II, res_omnes,
            clausura_instrumenti, optiones->fons->titulus);
        ordo_probandi = silex_ordinem_probandi_vitreum_fingere(piscina,
            optiones->titulus, fontes_prob, II, clausura_probationis,
    ...
    si (ad_discum)
    {
        DirectoriumFiltrum filtrum;
                    chorda radix_absoluta = via_absoluta(
                        chorda_ex_literis(proiectum_dir, piscina),
                        piscina);
                   ...
                   b32  scribere)
{
    ...
    per (index = 0; index < xar_numerus(clausura);
        index = index + 1)
    {
                  SilexRes* e = (SilexRes*)xar_obtinere(clausura,
                      index);
        constans character* via_plena = _texere(piscina,
            chorda_ut_cstr(radix_absoluta, piscina), "/",
