                    ...
                    i32* indiget_llm_extra)
{
    ...
    dum (tabula_dispersa_iterator_proximum(&iter, &clavis, &valor))
    {
        ...
        si (!liber->parsatum && liber->fila != NIHIL)
        {
            ...
            {
                FilumInfo* fi = (FilumInfo*)xar_obtinere(liber->fila,
                    i);
                si (!fi->in_old)
                {
        ...
        {
            StmlNodus* nodus_tit = stml_elementum_creare(piscina,
                intern, "titulus");
            {
        ...
        {
            StmlNodus* nodus_auc = stml_elementum_creare(piscina,
                intern, "auctor");
            {
                StmlNodus* textus_nodus = stml_textum_creare_ex_chorda(
        ...
        si (liber->fila != NIHIL)
        {
            ...
            per (i = 0; i < num_fila; i++)
            {
                ...
                stml_attributum_addere_chorda(nodus_filum, piscina,
                    intern,
                                              "via", fi->via);
                si (fi->in_old)
                {
                    stml_attributum_boolean_addere(nodus_filum, piscina,
                        intern,
                                                   "in_old");
                }
                si (fi->duplicatum)
