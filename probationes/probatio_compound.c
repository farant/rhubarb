     ...
     constans character* input)
{
    ...
    Arbor2Lexator* lex = arbor2_lexator_creare(p, intern, input,
        (i32)strlen(input));
           chorda* via;
     ...
     constans character* input)
{
    ...
    Arbor2GLRResultus  res = arbor2_glr_parsere_translation_unit(glr,
        tokens);
    chorda* output;
    imprimere("'%s'\n  -> ", input);
    si (!res.successus)
    ...
    si (   output->mensura == (i32)strlen(input)
        && memcmp(output->datum, input, (size_t)output->mensura)
                                                                           == 0) imprimere("OK\n");
    alioquin imprimere("MISMATCH: '%.*s'\n", (integer)output->mensura,
                 output->datum);
}
