...
principale(s32 argc, character** argv)
{
    ...
    {
        fprintf(stderr,
            "Nulla imago. Preme V pro paste ex clipboard.\n");
    }
    ...
    dum (currens && !fenestra_debet_claudere(fenestra))
    {
        ...
        dum (fenestra_obtinere_eventus(fenestra, &eventus))
        {
            ...
            si (dialogus_activus && dialogus_importatio != NIHIL)
            {
                ...
                dlg =
                    dialogus_importatio_obtinere_dialogum(dialogus_importatio);
                si (dlg != NIHIL && dlg->tractare_eventum != NIHIL)
                {
                    ...
                    si (fructus == DIALOGUS_CONFIRMATUS)
                    {
                        ...
                        si (dialogus_importatio_obtinere_fructum(
                                dialogus_importatio, &indices, &lat,
                                &alt, &tit))
                        {
                            fprintf(stderr,
                                "Imago salvata: %.*s (%d x %d)\n",
                                (integer)tit.mensura, tit.datum, lat,
                                alt);
                        }
                ...
                casus EVENTUS_CLAVIS_DEPRESSUS:
                    {
                        ...
                        alioquin si (k == 'v' || k == 'V')
                        {
                            ...
                            si (clipboard_habet_imaginem())
                            {
                                ...
                                clipboard_fructus =
                                    clipboard_capere_imaginem(piscina);
                                     ...
                                     && !dialogus_activus)
                        {
                            ...
                            importatio_visus =
                                importatio_visus_creare(piscina);
                            si (importatio_visus != NIHIL)
                            {
                                importatio_visus_initiare_sessionem(importatio_visus,
                                    imago);
                                dialogus_importatio =
                                    dialogus_importatio_creare(piscina,
                                    importatio_visus);
        ...
        si (necesse_rescalare && habet_imaginem && imago != NIHIL)
        {
            ...
            {
                fprintf(stderr,
                    "Errore: non possum scalare imaginem\n");
                habet_imaginem = FALSUM;
            }
            necesse_rescalare = FALSUM;
        }
        ...
        si (   habet_imaginem && dithering_fructus.successus
            && rgba_dithered != NIHIL)
        {
            s32 dest_x = 300;  /* Post graticula colorum */
            s32 dest_y = ((s32)tabula->altitudo
                - (s32)imago_scalata.altitudo) / 2;

            per (y = 0; y < (s32)imago_scalata.altitudo; y++)
            {
                per (x = 0; x < (s32)imago_scalata.latitudo; x++)
                {
                    s32 src_idx = (y * (s32)imago_scalata.latitudo
                        + x) * 4;
                    s32 dx = dest_x + x;
                    s32 dy = dest_y + y;
                        ...
                        && dy >= 0 && dy < (s32)tabula->altitudo)
                    {
                        ...
                        insignatus character r =
                            (insignatus character)rgba_dithered[src_idx
                                + 0];
