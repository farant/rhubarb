...
| `./oratio/reticulum.sh <mensura.tsv> -genus LIS -aurum aurum-… -sortes thesaurus [-ubi t=v1,v2]… [-gradus …] [-praeter …] [-greges columna\|catena] [-machina]` | the partition lattice (T35): every column judged held out, leave one corpus out; single columns by gain, lattice of columns (duplicates, covering edges, width), greedy chain with and without the fold veto (and what stopped it), top pairs by interaction gain, per-group votes; reads the `COLUMNAE` header rows, refuses by name (exit 2); rows `RETICULUM-*` with `-machina` |
| `reticulum_recensio.py <mensura.tsv> <same flags>` | independent recount of the instrument's `RETICULUM-COLUMNA/-SORS/-CATENA` rows from the §7 definitions (plain dictionaries, never the C tool): `diff` of the two must be empty (T35 e: 146 rows equal) |
