...
| tranche | state |
|---|---|
| T37 a instrument + recount | done — counts, FINIS and OPTIONES rows, condensed report; both expected files regenerated by hand and green on the first run; recount equal on every kind; plant = `>=` in the gaining-fold count |
| T37 b sentence id, examples, third run, records | done — `sententia` (sent_id, else the first line number), `-exempla N`; the first examples law was WEAK (substrings, not placement) and its plant stayed green — strengthened to pin each example to its group line, then red/green; oracle plant (column declared, not printed) red on the width law; third run: recount = C on 224 + 245 + 203 rows, examples resolve to Seneca HercF 318/330/339, Ag 279/361, Tacitus Ger 192 |
