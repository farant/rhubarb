...
| 3b | `probatio_html_circuitus` (2026-09-15, Fran: "are we doing round trip byte for byte for the whole html5lib test corpus?") | EVERY html5lib case (1,708; script-on included; fragments through `html_arbor_parsare_fragmentum` with the html5lib context): direct emission byte-identical to the input for ALL, then the STML cycle (write, read, write, comparator, re-read emission = input); STML refusals counted by NAMED cause and pinned exactly — 36 NUL (the raw-form limit, 01M2KPJ0HW), 0 closing-sequence; any other cause red. The O7 annotations are proven here, not on hand-picked cases | an invented element pushed but not appended → its children's bytes vanish → red |
