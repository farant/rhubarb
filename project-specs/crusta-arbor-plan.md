...
**SPLIT 2026-09-16 (Fran agreed): P11a = the printer, the coctum gate and
`coctum.sh` (Steps 1, 2, the coctum plant, one commit); P11b = the
instrument, both goldens, the oraculum and differentia gates, the
failure-class loop (Steps 3–6, one commit).** P11 held about three
tasks and Step 5 had no fixed end; each half now gets its own red
birth. The second golden is `sanitas.txt` (the spec's `validitas.txt`
is the older name). Commit trailers name the model that ran.

### P11a — measured mechanics (bash 5.2.15, before building)

A probe harness (`f() {\n<case>\n}\ndeclare -f f`, `cat -e`) over ~90
snippets fixed the printer's SHAPE, not only its rules. `declare -f`
is not a per-node pretty printer; it is a stream machine with state:

- **Two modes.** Function mode (everything under a function body): a
  `;` or newline connector prints `;` + newline + indent; a group
  prints `{ ` newline … newline indent `}`. Substitution mode
  (`$( )`, `<( )`, `>( )` — bash 5.2 REPARSES them at parse time and
  stores the reprinted text; indentation restarts at 0): `;` prints
  `; `, a newline connector prints a bare newline, a group prints
  `{ a; b; }`; a function definition inside switches to function mode
  for its body. Backticks are never reparsed (verbatim).
- **Connection shape decides heredoc flushing.** Lists (`;` `&`
  newline) and `&&`/`||` nest LEFT, pipes nest RIGHT; `|&` becomes
  `2>&1` appended to the left command's redirections + `|`. A
  trailing `&` attaches to the last command when the list's last
  connector is `;`/newline.
- **Heredoc bodies.** Headers print inline with the redirections;
  inside a connection the bodies are DEFERRED until the next
  connector prints (` |`, ` && `, ` &`: connector, newline, each body
  + delimiter + newline, then a space; `;`: newline + bodies only),
  otherwise flushed right after the command (newline + bodies). A
  flush sets a heredoc flag that SUPPRESSES the next `;` connector
  once (`cat <<A; b; c` prints `b` with no `;`); a command with
  redirections clears it. `semicolon()` after a loop/if body adds `;`
  unless the last byte is `&` or newline.
- **Skip-indent counter.** A connection indents once; `&` with a
  second command, `&&`, `||`, `|` keep the next command on the line;
  `;` in function mode does not.
- **Rules.** `elif` = nested `else` + `if … fi`; `for ((;;))` →
  `((1; 1; 1))`, each clause loses LEADING whitespace only; `for i` →
  `for i in "$@";`; `for i in;` → `for i in ;`; loops `for …;\n do` but
  `while …; do`; `case w in ` + per clause newline, indent+4 patterns
  ` | ` + `)`, body at +8, newline, `;;`/`;&`/`;;&` (absent → `;;`),
  empty body = blank line, `esac`; `coproc` unnamed → `COPROC`;
  `time [-p]` before `!`, `!` toggles (`! ! a` → `a`); nested
  functions `function g () ` (top level `f () `), a non-group body is
  wrapped in `{ }`, a group body's redirections follow `}`; subshell
  `( ` … ` )` with NO extra indent; `[[ ]]` single spaces, a bare word
  in expression position is `-n word`, `( e )` spaced; `(( ))` text
  verbatim; redirections `> o` (fd shown unless default), `0<> s`
  always with fd, `1>&2` `0<&3` digit dups with fd, `>&-`/`<&-` →
  `1>&-`/`0>&-`, word dups `>&$x` verbatim, `{fd}> o`, `<<< w`,
  heredoc delimiter quoted → `'A'` (dequoted value), `<<-` strips
  leading tabs of body and delimiter lines; words: `$'…'` decoded and
  single-quoted (`'\''`), `$"…"` → `"…"`, line continuations removed
  (also in unquoted heredoc bodies), arrays `(1 2 [k]=v)` single
  spaced, comments and blank lines gone.
...
- [x] **Step 1: Write the failing coctum gate** (P11a) — inline cases with the
  ...
  *Executed 2026-09-16 (P11a):* 36 cases, every expected text generated by
  running bash (a scratch generator writes the C table; nothing typed by
  hand), one per mechanic of the list above, plus the contract (NIHIL
  root, empty and comment-only programs, a bare program at level zero,
  a root function in the `declare -f` form, a sentence node) and the byte
  law on every wrapped case.

- [x] **Step 2: Run and verify it fails; implement `crusta_coctum.c`** (P11a) — a
  ...
  *Executed:* not one function per genus but the stream machine of the
  measured mechanics (the per-genus shape cannot express deferred heredoc
  bodies or the `;`-suppressing flag). 36/36 on the first compile;
  `crusta_effugia_decoquere` added to `crusta_arbor.h` (the `$'…'`
  decoder was private). Instrument `./crusta/coctum.sh <x.sh>`. Scratch
  cross-check over the fixtures, wrapped as the oracle will: 91 of 112
...
- [ ] **Step 8: Commit** (P11a committed its half separately: printer,
  coctum gate, `crusta/coctum.sh`, the decoder accessor, docs.)
