...
- [ ] **O2b — the oracle-named cheap rises** (Fran, 2026-09-15: "burn
  ...
  - [x] **O2b-4 named entities** (2026-09-15): root-tier
    `lib/entitates_html` — `entitates_html_tabula.c` GENERATED by
    `tools/entitates_html_generare.sh` (`-probare` = fresh?) from
    Python's stdlib `html.entities.html5` (WHATWG's list, offline):
    2,231 names, 106 legacy forms without `;`, values up to 6 UTF-8
    bytes, byte-sorted; `entitates_html_quaerere` (binary search) and
    `entitates_html_decoquere` (HTML5 rules: numeric with optional
    `;`, C1 → Windows-1252, invalid → U+FFFD; named LONGEST match,
    legacy forms; the attribute exception before `=`/alnum). Root
    gate `probatio_entitates_html` (100 assertions). Consumers:
    `md_decoctum` keeps CommonMark's strict `name;` rule over the
    shared table (its 57-entry table gone; its output buffer grew —
    `&nGt;` is 5 bytes in, 6 out; computus golden regenerated with
    that cause; md oracle 1,270 → 1,272) and `html_coctum` decodes
    text, RCDATA and attributes with the full decoder (legacy forms
    now decoded in TEXTUS). **985 → 1,047.** A generated `.c` needs
    ITS OWN header for aedilis' include closure to link it. Plant:
