...
| `<@attr=>` | argument-element: block-form call argument (`@`-prefix + `=`-suffix tag), value = its CHILDREN — the named-subtree-argument form; a call consumes the maximal run of immediately-following argument-elements | **DECRETUM** 2026-08-26 (macros spec §6.1) — undesigned in detail, unbuilt |
