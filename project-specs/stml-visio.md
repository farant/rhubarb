...
| `<@attr=>` | argument-element: block-form call argument (`@`-prefix + `=`-suffix tag), value = its CHILDREN — the named-subtree-argument form; a call consumes the maximal run of immediately-following argument-elements | **DECRETUM** 2026-08-26 (macros spec §6.1) — undesigned in detail, unbuilt; generalization to ALL tags (parent-binding attribute-elements) UNDER CONSIDERATION |
| CAPS-Latin tags (`<COMMUTATIO>`, `<CASUS>`, `<ORDINARIUS>`) | BUILTIN-VERB grammar for the template layer — data tags stay lowercase kebab, builtins SCREAM. First verb: arm selection in definition bodies; cases = literals + presence-tests ONLY (the no-expression-language line), richer cases arrive as patterns | **DECRETUM** 2026-08-26 (macros spec §6.2) — unbuilt; brings optional slots `@p?` + narrowing |
