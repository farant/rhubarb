...
| `<<selector>>` | transclusion | **VIVIT-as-node** in C (`STML_NODUS_TRANSCLUSIO` — parsed, carried); **RESOLUTION = RESERVATUM** in the library (TS resolves with CSS selectors). First C consumer resolving `<<#id>>` to TEXT: **briar** (2026-09-05, briar-spec §3.4) — a `<<#x>>` line inside a `<c!>` region is the literate tangle's reference to the region `<c! id="x">`, alias semantics, resolved before silva parses; a line scan today, a lexer token when the `briar-c89` dialect arrives |
