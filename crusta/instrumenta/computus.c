/* computus.c - CLI computi crustae: ./crusta/computus.sh <plagula.sh>
    ...
    constans CrustaComputus* c)
{
    imprimere("# via\tocteti\tlexemata\tnodi\tocteti_stml\tsana"
              ...
              "\tms_arbor_legendi\tms_comparandi\tusus\tcommissa"
              "\totiosa"
    ...
    character** argumenta)
{
    ...
    {
        fprintf(stderr,
            "usus: computus <plagula.sh> [-machina] [-iter N]\n"
            "  -machina  ordo TSV (caput '#')\n"
            "  -iter N   mensura N vicibus (ultima impressa)\n");
        redde II;
    }
    piscina = piscina_generare_dynamicum("crusta_computus",
        MMMMXCVI * CCLVI);
    si (piscina == NIHIL)
    {
    ...
    {
        sana = crusta_computus_metiri((constans character*)fons,
            mensura,
            &c);
    }
    si (machina)
