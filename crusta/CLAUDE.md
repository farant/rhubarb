...
- **Gates (15, `./crusta/compile_probationes.sh`, every one born red by
  ...
  | exempla | the case reader contract; both fixtures and the FreeBSD manifest pinned (89 files, 17,297 bytes); 36 pathology cases, last = `arith-operandum-absens` | `## END` kept in the section |
  ...
  | stml | 490 documents (112 inline, 53 fixture cases, 325 files) through two cycles: texts byte-equal, comparator STRUCTURALIS and FIDELITAS (asserted), heredoc references carried, re-read emission = source; raw-form and mixed-edge refusals pinned by cause | the re-read program's `cauda` removed |
  ...
  (47 → 51 judged), totalitas (33 → 37 CRLF cases), both oracle counts
  and the computus row for `pathologiae.sh` — one commit, each with its
  cause.
  ...
  | oraculum | our cooked view vs golden `declare -f` over 115 cases: pares ≥ **106** (rising pin; birth 92), counts pinned, no orphan golden | pin raised one above the count |
  | diagnostica | declared shapes vs the four counters, class by class: house 235, oracle 125, adversarial 17, 16 inline shapes, plus every fuzz case | VACUA counted as a closure; `malum`'s `inanis` removed |
  | differentia | `relatio.sana` vs golden `bash -n` over 125 cases: concordes ≥ **118** (rising; birth 115); and, from the DERIVED diagnostics, "no erratum" vs bash exit 0 ≥ **122** and the first erratum's line vs bash's `line N` ≥ **7**/9; every disagreement named; house-corpus mala ≤ **0** (falling pin; fixtures excluded as in corpus) | sana inverted for zero-mala parses (concordes 6); every `gravitas="monitum"` removed (gravitas 122 → 118, concordes UNMOVED) |

- **Diagnostica (2026-09-17, materia-sedes B3).** Error shapes are
  DECLARED in `grammatica/crusta.registrum.stml` (`absentia` /
  `vacua` on loci, `diagnosticum` on `malum`, `inanis` on
  `separator`/`heredoc`/`malum`) and derived by materia's one walker;
  `crusta_diagnostica(piscina, radix, relatio)` returns located
  records, `crusta_diagnostica_concordia` checks them against the four
  counters (presence per class; `sana` exactly "no diagnostics"). No
  genus or locus was added, so the seal did NOT move. The `elif`
  empty-condition class has no declarable trace and is emitted by the
  builder into `CrustaParsura.diagnostica`. `separator`/`heredoc`
  `inanis` are unexercised by every corpus (only `malum`'s carries) —
  kept to mirror `_lista_plena`, named in the worklog.

  **Step 6 (the same day) put the derived diagnostics beside bash.**
  Two rising pins in `differentia`: "no erratum" against bash's exit 0,
  and the first erratum's line against the `line N` in bash's own error
  text. The line pin reads 7/9 and the two it misses are a complete law
  — every case where bash names a TOKEN agrees; two of the three where
  bash names `unexpected end of file` differ, because bash gives the EOF
  line and we give the end of the unfinished construct. That is a
  choice, not a defect. The gravitas pin needed the corpus widened
  first: it read 118 — identical to `relatio.sana`'s own pin, case for
  case — and removing EVERY `gravitas="monitum"` in the declaration
  could not move it, because no case carried a monitum without also
  carrying an erratum. The four shapes D9 measured for exactly this
  (`[[ a b ]]`, `(( 1 2 ))`, `(( 3 ! ))`, `$(( 1 + ))`: bash warns,
  exit 0) existed only as inline shapes in `diagnostica`, never as
  oracle cases. Added to `pathologiae.sh`, they lift gravitas to 122
  while `concordes` stays 118 — and the gap of four IS the monita. The
...
and — until 2026-09-18 — `silva.exemplaria`, now DELETED: tier-2
