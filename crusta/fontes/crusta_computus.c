/* crusta_computus.c - imago memoriae et temporis parseris crustae
 * (vide .h) */
#include "latina.h"
#include "piscina.h"
#include "chorda.h"
           ...
           CrustaComputus* exitus)
{
    ...
    piscina = piscina_generare_dynamicum("crusta_computus_parsare",
        CRUSTA_COMPUTUS_ALVEUS_INITIUM);
    si (piscina == NIHIL)
    {
        redde FALSUM;
    }
    ...
    arbor_idem   = FALSUM;
    {
        ...
        altera = piscina_generare_dynamicum("crusta_computus_arbor",
            CRUSTA_COMPUTUS_ALVEUS_INITIUM);
        si (altera == NIHIL)
        {
            piscina_destruere(piscina);
            ...
            &iudicium))
        {
            materia_arbor_consilium_nudum(&consilium, &CRUSTA_REGISTRUM,
                &ratum, "crusta");
            /* cauda lexematis crustae (profunditas backtick) - lector
             * formam eandem accipit (P7) */
            consilium.forma.mensura_caudae =
                (i32)magnitudo(CrustaCauda);
            t0 = clock();
