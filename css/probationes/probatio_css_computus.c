/* probatio_css_computus.c - PORTA COMPUTI CSS: numeri deterministici
 * corporis fixi (fixa/css) contra aurum pinnati (nodi, lexemata,
 * octeti STML, memoria piscinae parsurae, allocationes); tempora
 * imprimuntur, non pinnantur. Gemellus probatio_silva_computus.
 *
...
#define COLUMNAE   11
#define NUMERUS_CORPORIS 4

hic_manens constans character* CORPUS[NUMERUS_CORPORIS] = {
    "componentia_2026-08-18.css",
    "thema_2026-08-18.css",
    "adversarius.css",
    "adversarius_2.css"
};
...
principale (vacuum)
{
    ...
    piscina = piscina_generare_dynamicum("probatio_css_computus",
        MMMMXCVI * XVI);
    si (!piscina)
    {
    ...
    sprintf(via_auri, "%s/css/probationes/fixa/computus/basis.tsv",
        radix);

    imprimere("\n--- I. mensura corporis (fixa/css) ---\n");
    per (i = ZEPHYRUM; i < NUMERUS_CORPORIS; i++)
    {
        ...
        metita[i] = FALSUM;
        sprintf(via, "%s/probationes/fixa/css/%s", radix, CORPUS[i]);
        ...
        (vacuum)css_computus_metiri((constans character*)fons, mensura,
            &c);
        _columnas(&c, facta[i]);
        metita[i] = VERUM;
