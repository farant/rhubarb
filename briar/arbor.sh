#!/bin/bash
# briar/arbor.sh - proiectio STML plagulae thistle
#
# Usage:  ./briar/arbor.sh <plagula.thistle> [-tacitus]
...
declare -a INCLUDE_FLAGS=("-I$RADIX_DIR/include" "-I$MATERIA_DIR/fontes" "-I$MD_DIR/fontes" "-I$BRIAR_DIR/fontes")
SRC="$BRIAR_DIR/instrumenta/arbor.c"
if [ ! -d "$BUILD_DIR" ] || [ -z "$(ls "$BUILD_DIR"/briar_*.o 2>/dev/null)" ]; then
    echo "arbor: obiecta absunt - ./briar/compile_probationes.sh registrum primum" >&2
    exit 2
fi
OBJ=""
