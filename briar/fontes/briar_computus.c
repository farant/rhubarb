/* briar_computus.c - vide briar_computus.h (gemellus css_computus.c) */
            ...
            BriarComputus* exitus)
{
    ...
    piscina = piscina_generare_dynamicum("briar_computus_parsare",
        BRIAR_COMPUTUS_ALVEUS_INITIUM);
    si (piscina == NIHIL)
    {
        redde FALSUM;
    }
    ...
    _numerare(radix, &exitus->nodi, &exitus->lexemata);
    {
        ...
        altera = piscina_generare_dynamicum("briar_computus_arbor",
            BRIAR_COMPUTUS_ALVEUS_INITIUM);
        si (altera == NIHIL)
        {
            piscina_destruere(piscina);
        ...
        materia_scriptura_consilium_nudum(&cs, &BRIAR_REGISTRUM);
        cs.fons_index = BRIAR_FONS_PLAGULAE;
        t0 = clock();
