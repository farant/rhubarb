 ...
 * Grammatica exterior thistle per constructionem totalis est: regio
 * non clausa usque ad finem, clausura vaga prosa, saeptum apertum
 * vitium documenti - nulla plagula recusatur.
 *
 * GENERATORES (ut md): octeti fortuiti (LCG seminatus), mutatio
 * corporis fixi (I ex XL octetis), truncatio (XXIII gradus),
 * nidificatio formis thistle (regiones, elementa, saepta, prosa,
 * mixtum, CRLF) clausa et aperta profunditatibus I..M, deinde L milia.
 *
 * IN FRACTURA: fons peccans in briar/build/totalitas_fractum.thistle.
 */
                   ...
                   i32  mensura)
{
    ...
    piscina = piscina_generare_dynamicum("briar_totalitas_casus",
        65536);
    ...
    emissa = briar_emittere(piscina, radix);
    si (!emissa.successus)
    {
                   ...
                   i32  mensura)
{
    ...
    sprintf(via, "%s/briar/build/totalitas_fractum.thistle",
        RADIX_VIAE);
    f = fopen(via, "wb");
    si (f == NIHIL)
    {
...
principale (vacuum)
{
    ...
    piscina = piscina_generare_dynamicum("probatio_briar_totalitas",
        1048576);
    ...
    /* ==================================================
     * PROFUNDITAS INGENS: L milia - regio una non clausa cum C milibus
     * linearum, et saepta C milia commutantia: quaesitio clausurae
     * linearis, cursor fabricae resumens - NON RUIT asseritur
     * ================================================== */
