#!/bin/bash
# md/html.sh - markdown -> html per compositionem STML (B2)
#
# Usage:  ./md/html.sh <plagula.md> [-stml]   (-stml = documentum compositum)
...
SRC="$MD_DIR/instrumenta/html.c"
if [ ! -d "$BUILD_DIR" ] || [ -z "$(ls "$BUILD_DIR"/md_*.o 2>/dev/null)" ]; then
    echo "html: obiecta absunt - ./md/compile_probationes.sh registrum primum" >&2
    exit 2
fi
OBJ=""
...
if [ ! -f "$BIN" ] || ! [ "$BIN" -nt "$SRC" ] || [ -n "$(find "$BUILD_DIR" -name 'md_*.o' -newer "$BIN" 2>/dev/null)" ]; then
    rm -f "$BIN"
    clang "${GCC_FLAGS[@]}" "${INCLUDE_FLAGS[@]}" "$SRC" $OBJ -o "$BIN" || exit 1
fi
exec "$BIN" "$@" -programma "$MD_DIR/html/md-html.stml"
