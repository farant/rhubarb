/* probatio_md_computus.c - PORTA COMPUTI MARKDOWN: numeri deterministici
 * corporis fixi (fixa/md - snapshots, FONTES.md) contra aurum pinnati (nodi, lexemata,
...
principale (vacuum)
{
    ...
    sprintf(via_auri, "%s/md/probationes/fixa/computus/basis.tsv",
        radix);

    imprimere("\n--- I. mensura corporis (fixa/md) ---\n");
    per (i = ZEPHYRUM; i < NUMERUS_CORPORIS; i++)
    {
        ...
        metita[i] = FALSUM;
        sprintf(via, "%s/md/probationes/fixa/md/%s", radix, CORPUS[i]);
        ...
        (vacuum)md_computus_metiri((constans character*)fons, mensura,
            &c);
        _columnas(&c, facta[i]);
        metita[i] = VERUM;
