...
# Effusio cursus ultimi semper in build/test_logs/md.log
# radicis (stdout + stderr; codex exitus per PIPESTATUS servatus).
if [ -z "${MD_PROBATIONES_EFFUSIO:-}" ]; then
    export MD_PROBATIONES_EFFUSIO=1
    mkdir -p "$(dirname "$0")/../build/test_logs"
    "$0" "$@" 2>&1 | tee "$(dirname "$0")/../build/test_logs/md.log"
    exit "${PIPESTATUS[0]}"
fi

# md/compile_probationes.sh - Probationes markdown (cliens tertius materiae)
#
# Usage:
...
MATERIA_DIR="$(cd "$MD_DIR/../materia" && pwd)"
...
done < <(find "$RADIX_DIR/include" "$MD_DIR/fontes" "$MD_DIR/probationes" \
             -name "*.h" 2>/dev/null)
...
for src in "$MD_DIR"/probationes/*.c; do
    base="$(basename "$src" .c)"
...
# corpus markdown = plagulae a git TRACTATAE ('git ls-files' - ambulatio
...
mensor_suitae_incipere "md."
total=0 ; passed=0 ; failed_names=""
for test_file in "$MD_DIR"/probationes/probatio_*.c; do
...
echo "========================================"
echo "MD PROBATIONES: $passed/$total praeteritae"
