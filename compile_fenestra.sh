...
do_compile() {
    echo -e "\033[34m======================="
    echo "COMPILING FENESTRA"
    echo -e "=======================\033[0m"

    # Compile fenestra_textus.c
    echo "Compiling fenestra_textus.c..."
    if ! clang ${CLANG_FLAGS[@]} ${INCLUDE_FLAGS[@]} -c lib/fenestra_textus.c -o /tmp/fenestra_textus.o; then
        echo -e "\033[31mFailed to compile fenestra_textus.c\033[0m"
        return 1
    fi

    # Compile fenestra_macos.m (no framework needed for compilation, only linking)
