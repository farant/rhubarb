...
# silva/origo.sh - fabula expansionis (instrumenta prima #3)
#
# Usage:
#   ./silva/origo.sh <plagula> <linea>[:<columna>]
# Flags: -omnia (tectum mensurae sublatum)
# Exit:  0 inventum | 1 nihil ad positionem | 2 sine arbore / usus
#
...
source "$SILVA_DIR/origo_fontes_generata.sh"
...
ORIGO_SRC="$SILVA_DIR/instrumenta/principalia/origo.c"
ORIGO_BIN="$BUILD_DIR/origo"
clang "${GCC_FLAGS[@]}" "${INCLUDE_FLAGS[@]}" "$ORIGO_SRC" $obj_files \
    -o "$ORIGO_BIN" || exit 1
