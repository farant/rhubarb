...
/* ==================================================
 * latina.h praebere custode neutralizato (vide caput)
 * ================================================== */

/* lineam [a, b) vacuare (novae/CR servatae - positiones
 * linearum immutatae) */
interior vacuum
_lineam_vacuare (
    i8* datum,
...
/* congruitne linea (spatiis ductis omissis) cum praefixo, quo
 * sequente fine lineae aut spatio? */
interior b32
_linea_congruit (
    constans        i8* datum,
                   i32  a,
    ...
    constans character* praefixum)
{
    ...
    dum (a < b && (datum[a] == ' ' || datum[a] == '\t')) a += I;
    n = (i32)strlen(praefixum);
    si (b - a < n) redde FALSUM;
    per (i = ZEPHYRUM; i < n; i += I)
    {
        si ((character)datum[a + i] != praefixum[i])
        {
            redde FALSUM;
        }
    }
    si (a + n == b) redde VERUM;
    redde datum[a + n] == ' ' || datum[a + n] == '\t'
        || datum[a + n] == '\n' || datum[a + n] == '\r';
}
                   ...
                   i32  mensura)
{
    ...
    si (!contextus || !textus || mensura <= (i32)ZEPHYRUM)
    {
        redde FALSUM;
    }
    copia = (i8*)piscina_allocare(piscina,
        (memoriae_index)mensura);
    si (!copia) redde FALSUM;
    memcpy(copia, textus, (memoriae_index)mensura);

    ifndef_vacuatum = FALSUM;
    define_vacuatum = FALSUM;
    endif_a         = ZEPHYRUM;
    endif_b         = ZEPHYRUM;
    initium         = ZEPHYRUM;
    per (i = ZEPHYRUM; i <= mensura; i += I)
    {
        si (i < mensura && copia[i] != '\n') perge;
        {
            i32 finis;

            finis = i < mensura ? i + I : i;
            si (!ifndef_vacuatum && _linea_congruit(copia,
                initium, finis, "#ifndef LATINA_H"))
            {
                _lineam_vacuare(copia, initium, finis);
                ifndef_vacuatum = VERUM;
            }
            alioquin si (ifndef_vacuatum && !define_vacuatum
                && _linea_congruit(copia, initium, finis,
                    "#define LATINA_H"))
            {
                _lineam_vacuare(copia, initium, finis);
                define_vacuatum = VERUM;
            }
            alioquin si (_linea_congruit(copia, initium, finis,
                "#endif"))
            {
                endif_a = initium;
                endif_b = finis;
    ...
    /* '#endif' ULTIMUM = par custodis - vacuatur solum si custos
     * inventus (sine custode textus integer praebendus) */
    si (ifndef_vacuatum && define_vacuatum
        && endif_b != (i32)ZEPHYRUM)
    {
        _lineam_vacuare(copia, endif_a, endif_b);
    }

    redde silva_contextus_praebere(contextus, "latina.h",
        (constans character*)copia, mensura);
}
