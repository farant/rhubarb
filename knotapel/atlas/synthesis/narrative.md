...
*Updated through Demo 109 (2026-02-26)*
...
- **Barrington-Radical Principle**: the radical of W_{6,4} carries an abelian writhe character (M*r = A^{writhe}*r for all 32,768 entries); by Barrington's theorem, provably useless for parity; simple W_{6,0} outperforms at every XOR level (Demo 102).
- **Writhe Character Theorem**: radical content constant = 3 for all 32,768 W_{6,4} entries; the radical is "frozen" to writhe data only (Demo 102).
- **BFS growth is a braid group invariant**: all three n=6 modules (W_{6,0}, W_{6,2}, W_{6,4}) produce bit-for-bit identical BFS depth profiles; growth rate ~(n-1)x confirmed across five strand counts n=3,4,5,6,8 (Demos 102-103-105).
- **D93's "XOR dies at N>=7" refuted**: k-regime artifact; at k=4096, W_{8,0} produces XOR8=22266 (Demo 105).
- **Topological entropy null**: both periodic and pseudo-Anosov braids compute XOR at 100% participation; entropy is orthogonal to all 8 Boolean functions tested; the TL visibility filter at delta=0 erases dynamical information (Demo 106).
- **Z/4Z Axis-Alignment Theorem**: every braid representation matrix entry over a TL module at delta=0 is axis-aligned; proven constructively (Demo 107).
- **Nesting Parity Z/2Z Grading**: nesting count mod 2 = bipartite 2-coloring for all TL link-state transition graphs; 195 matchings, 0 mismatches (Demo 107).
- **q-Catalan identity**: C_{2m+1}(-1) = (-1)^m * C_m; proved algebraically and verified at n=2 through 6 (Demo 107).
- **Dual-Channel Theorem**: parity requires BOTH multiplicative phase coherence AND additive magnitude diversity; maximum structural contrast between parity (17 product edges) and poison (0 product edges) (Demo 108).
- **Encoding-Dependent Dual-Channel Theorem**: product closure polarity inverts between additive encoding (parity wants HIGH) and multiplicative encoding (parity wants LOW) (Demos 108-109).
...
- Radical-as-Resource Hypothesis REFUTED: simple W_{6,0} outperforms non-simple W_{6,4} at every XOR level despite identical dimension; the mixing row provides an 8% boost over bare quotient but does not overcome the simple module (Demo 102).
- Fibonacci max-abs for non-simple module: W_{6,4} max coefficient magnitude follows strict Fibonacci (1,1,2,3,5,8,13) through depth 6 while simple W_{6,0} breaks to 16; the radical constrains coefficient growth (Demo 102).
- Curse of dimensionality in sign-hash: W_{6,2} (dim=9, 324 components) is strictly worse than W_{6,0} (dim=5, 100 components) at every XOR level under full sign-hash activation (Demo 103).
- Sign-rank expansion: W_{6,2} sign-rank 292 exceeds raw rank 244 by 48 directions; W_{8,2} sign expansion 1.83x and growing at 16384 entries (Demos 104-105).
- Branching interaction dominance at n=6: cross-block (off-diagonal) outperforms within-block (diagonal) for asymmetric decomposition dim 5+4 (Demo 104).
- Branching interaction REVERSAL at n=8: within-block dominates for symmetric decomposition dim 14+14 (Demo 105).
- k-regime theory confirmed at n=8: the ratio k/2^N controls XOR viability across both n=6 and n=8 (Demo 105).
- Rank saturation confound: all prior cross-module rank comparisons are lower bounds until saturation-verified (Demo 105).
- Fibonacci pseudo-Anosov at depth 2: all 8 depth-2 pA entries share characteristic polynomial x^3-4x^2+4x-1 with entropy/crossing log(phi) (Demo 106).
- Caruso genericity confirmed: pA fraction crosses 50% at depth 5, reaches 77.2% at depth 8 (Demo 106).
- Bipartiteness of TL link-state transition graphs: verified for all TL_n left regular (n=2..6) and 6 standard modules (Demo 107).
- Major Index Bipartite Invariant: maj mod 2 = bipartite coloring, verified at W_{4,2} (3/3) and W_{6,2} (9/9) (Demo 107).
- Orthogonality of multiplicative/additive quotient graphs: product = K_5 (Z/4Z), additive = star tree (Demo 107).
- 2-adic neutrality: v_2 distribution of TL matrix entries decays geometrically ~0.5x per level (Demo 107).
- Product closure as maximum-contrast parity discriminator: 17 edges vs 0 edges (Demo 108).
- Partition coincidence as incapacity signature: triple collapse root=Galois=norm in poison set (Demo 108).
- Perfect 7/8 structural symmetry between parity and non-parity at delta=sqrt(2) (Demo 109).
...
- Macrame principle crossover: the branching interaction regime transition occurs between block dim 5 (cross-block wins, D104) and dim 14 (within-block wins, D105); where is the exact crossover? (Demo 105).
- Atkinson peak k-dependence: at k=128 the peak is 120 components; does it shift at higher k? (Demo 104).
- Sign-rank expansion scaling: expansion ratio increases with module dimension and decreases with oversampling; W_{8,2} at 1.83x still growing; what determines the asymptotic value? (Demo 105).
- General cross-block recipe: for W_{n,j}, is the optimal activation always the cross-block under TL_{n-1} branching? D105 reversal suggests not — depends on block complexity. (Demos 104-105).
- Effective dimension hypothesis: the key metric may be action_density = effective_dim/dim^2; W_{6,0} (j=0) may have highest action density because all TL generators create/destroy arcs with no through-lines to preserve (Demo 103).
- Does the topological entropy null extend to higher strand counts? Expected yes since the TL visibility filter argument is representation-independent (Demo 106).
- Major Index Bipartite Invariant for general W_{n,j}: verified at W_{4,2} and W_{6,2}; need W_{8,2}, W_{6,4} etc. for more evidence (Demo 107).
- 2-adic neutrality formal proof: five converging frameworks identified (Kummer carry-counting, nilpotent TL, Goodman-Wenzl, unipotent, Stirling) — need to formalize (Demo 107).
- Does the value set converge? 186 values at depth 8 with 72 new; need deeper catalogs beyond 32768 cap (Demo 107).
- Dual-channel prediction for XOR6: can dual-channel analysis predict which custom sector labelings at k>=24 achieve XOR6? (Demo 108).
- Quantitative product-closure threshold: how much product closure and v_2 connectivity does a value set need for parity? Is there a sharp threshold? (Demo 108).
- Additive encoding at delta=sqrt(2): does holding delta constant and varying encoding restore D108-style structural discrimination? Would directly confirm encoding-dependence (Demo 109).
- j=0 liveness implies structural universality: at delta!=0, rich algebra implies Raqiya cannot discriminate parity from non-parity; conjecture extends D52's j=0 sector liveness theory (Demo 109).
- 50% vocabulary as structural constant at delta=sqrt(2): is the 28/28 = 50% split an artifact of Re>0 or universal across all activations? (Demo 109).
...
- Module-specific dual-channel test: does the dual-channel distinction hold for module-specific value sets (e.g., W_{8,0} vs W_{8,2})? (Demo 108).
- Z[i] product closure at delta=sqrt(2): does the Z[i] lattice have intrinsically better product closure than Z[zeta_8]? Would explain D55's 9334-solution explosion (Demo 109).
- Raqiya Z[zeta_16] full extension: is it worth building full Raqiya support for Z[zeta_16] if more delta!=0 analysis is needed? (Demo 109).
- Macrame principle crossover location: W_{6,4} or W_{10,2} could bracket the block-dimension threshold between cross-block and within-block dominance (Demo 105).
- W_{8,2} rank saturation at full oversampling: at 16384/3136 = 5.2x, W_{8,2} is at 34.9% raw rank; ~130K entries needed for fair comparison (Demo 105).
- Major index theorem proof for general n,j: verified computationally at two modules; algebraic proof open (Demo 107).
- 2-adic neutrality formal proof: select and formalize one of five identified frameworks (Demo 107).
- Strongly regular graph parameters in Raqiya edge subgraphs: some regular subgraphs may have (n,k,lambda,mu) parameters (Demo 107).
...
## 17. The Multi-Strand Scaling Arc: Radical Refutation and Activation Diagnostics

Demos 98-101 established that multi-strand DKC works — that matrix-valued braid representations compute XOR via sign-hash activation, with BFS growth tracking n-1 and non-semisimple modules outperforming simple ones at the hardest XOR levels. Demos 102-105 constituted a systematic four-demo arc that pressure-tested every moving part of the multi-strand framework at n=6 and n=8, yielding a definitive refutation of the radical-as-resource hypothesis, settling the growth-law question, diagnosing the activation bottleneck, and discovering a regime transition in branching interaction structure.

### The Barrington-Radical Principle (Demo 102)

The multi-strand arc's central question had been sharpening since Demo 100: does the radical of a non-semisimple TL module provide computational advantage for parity? Demo 101 showed that the simple W_{5,3} achieved XOR14=0 while the non-semisimple W_{4,2} achieved XOR14=70. But strand count, module dimension, and radical structure were all confounded. Demo 102 isolated the radical as the sole variable: W_{6,0} (dim=5, simple, radical dim=0) versus W_{6,4} (dim=5, non-simple, radical dim=1), both on the same 6-strand braid group with identical dimensionality.

The result was a landmark refutation. The simple module W_{6,0} won at *every* XOR level: XOR6 (2449 vs 2370), XOR8 (850 vs 793), XOR10 (1 vs 0). The radical carries an abelian writhe character — M*r = A^{writhe(M)}*r for all 32,768 catalog entries — which by Barrington's theorem (1989) is provably useless for parity computation. Abelian groups cannot compute parity. The radical is "frozen": it remembers only the total writhe of the braid word, acting as a 1-dimensional representation through the Z/8Z group of A-powers.

This named result — the **Barrington-Radical Principle** — connects TL representation theory directly to computational complexity theory. The radical content (L1 norm of M*r) is constant at 3 across all 32,768 entries. The radical direction has constant magnitude regardless of braid word length. The max coefficient magnitude follows strict Fibonacci (1,1,2,3,5,8,13) through depth 6 in the non-simple module but breaks to 16 in the simple module — the radical constrains coefficient growth, another manifestation of its abelian character.

The nuance: B_full (2370) exceeded B_quotient-only (2195) at XOR6, an 8% boost from the mixing row. The mixing row encodes how the quotient block leaks into the radical direction. So the radical is not entirely inert — it provides a small boost relative to the bare quotient — but the 5-dimensional simple module beats the 4+1 decomposition overall. The mixing row is a weak secondary resource, not a primary computational mechanism.

### BFS Growth Is a Braid Group Invariant (Demo 103)

If the radical does not help, does module dimension? Demo 103 compared W_{6,0} (dim=5) against W_{6,2} (dim=9), both simple modules on B_6. The BFS depth profiles were bit-for-bit identical: 1, 11, 77, 439, 2233, 10603, 32768 — same entries at every depth, same max coefficient magnitudes, same growth ratio converging near 5x. Combined with Demo 102's identical profile for W_{6,4}, all three n=6 modules produce the same BFS tree. Growth rate is a property of the braid group B_n, not the module.

This settles the growth law definitively. The hypothesis that higher-dimensional modules might have ~dim(W)x growth was eliminated: 9x9 matrices produce no more distinct products than 5x5 matrices at the same braid depth. The BFS catalog is the braid group's fingerprint, and the module is merely a window through which it is observed.

Yet dim-9 hurt XOR capacity at every level: XOR6 (2238 vs 2449), XOR8 (557 vs 850), XOR10 (0 vs 1). The curse of dimensionality in sign-hash activation was confirmed — 324 sign components (9x9 x 4) create a sparser hash space than 100 components (5x5 x 4), reducing collision-based XOR capacity. More representation dimensions means more hash bits wasted on redundant information.

The surprise came from the subset-hash experiment. B@sub7 — hashing 196 components from a 7x7 subblock of the 9x9 representation — produced XOR6=2538, *beating* A@full's 2449. The 7x7 subblock of W_{6,2} contains algebraic information that W_{6,0} simply does not have. B@stride3 (~108 components, every 3rd entry) achieved XOR8=767, approaching A@full's 850 while using a completely different sampling geometry. The deficit was not in the algebra; it was in how the activation function read the algebra. This set the stage for Demo 104's systematic investigation of activation coarseness.

### The Activation Is the Bottleneck (Demo 104)

Demo 104 asked: if the XOR deficit between dim-5 and dim-9 is a hash problem, not an algebra problem, can the hash be tuned to recover the dim-9 module's algebraic advantage?

The answer came in a non-monotonic curve. Sweeping component count from 10 to 324 for W_{6,2} at k=128, the XOR6 score rose to a peak at 120 components (XOR6=2486, beating W_{6,0}'s maximum 2449), then declined through the middle range, partially recovered at 300, and dropped again at 324. The **Atkinson sweet spot** — named by analogy to Atkinson dithering — is the optimal tradeoff between information density and hash-collision structure. Too few components loses signal; too many dilutes the useful collisions that enable XOR computation.

The rank analysis explained the mechanism. W_{6,0} has raw rank 100/100 — every component is linearly independent. W_{6,2} has raw rank 244/324 — 80 components are linearly dependent over Z. But sign quantization *expands* the rank: W_{6,2}'s sign-rank is 292/324, a gain of 48 independent directions. The nonlinear sign() function breaks linear dependencies that exist over Z, creating new independent ternary patterns. This **sign-rank expansion** is a genuinely new finding — sign quantization is not merely a lossy compression but can be an information-expanding nonlinear projection.

The branching decomposition provided the structural insight. Under TL_5 restriction, W_{6,2} decomposes into a through-strand piece (rows 0-4, dim=5) and an arc-bound piece (rows 5-8, dim=4). The **cross-block** (off-diagonal, 160 components) achieved XOR6=2412 and XOR8=952; the **within-block** (diagonal, 164 components) achieved only XOR6=2235. The computational signal lives in the interaction between the two TL_5 sub-pieces, not within either piece alone. The branching interaction IS the signal — at n=6.

But the k-sweep revealed a critical qualification. At k=4096, W_{6,0} (100 components) beat W_{6,2} cross-block (160 components) at every XOR level. The cross-block advantage at k=128 was a low-k pigeonhole artifact. Multi-hash architecture provided no improvement either. The conclusion: **k (cell count) is the real lever**, not hash architecture. The cross-block advantage is a better *activation design* (extracting more information per cell) but the dim-5 module has more fundamental capacity at sufficient k.

### Scaling to 8 Strands and the Macrame Principle (Demo 105)

Demo 105 extended the multi-strand framework to n=8 — 14-dimensional W_{8,0} (simple, C_4=14 link states) and 28-dimensional W_{8,2} (simple, symmetric branching into two 14-dimensional blocks). The first major result: D93's "XOR dies at N>=7" was **refuted** as a k-regime artifact. At k=4096, W_{8,0} produces XOR8=22266, beating n=6's 21699 at matched k. The ratio k/2^N is the real control parameter: at k=128 for N=8, k/2^N=0.5 (deep pigeonhole regime), but at k=4096, k/2^N=16 (collision avoidance regime), and XOR is fully alive.

BFS growth continued the sl_d functor thesis: 14 generators on B_8 fill the catalog faster, hitting 32768 at round 5 (vs round 6 for B_6). The fourth data point — ~7x growth rate for n=8, compared to ~2x (n=3), ~3.1x (n=4), ~4x (n=5), ~5x (n=6) — now confirms the pattern across five strand counts.

The branching decomposition produced the arc's most unexpected result. At n=8, W_{8,2} decomposes symmetrically under TL_7 into two 14-dimensional blocks (W_{7,1} + W_{7,3}). The within-block activation (1568 components) achieved XOR6=2940, XOR8=4743; the cross-block activation (1568 components) achieved only XOR6=532, XOR8=733. Within-block dominates — the *opposite* of Demo 104's n=6 finding where cross-block dominated.

This reversal crystallized the **macrame principle**: at n=6 with asymmetric blocks (dim 5+4), the sub-modules are individually too simple for rich computation, so their cross-block interaction is the dominant resource. At n=8 with symmetric blocks (dim 14+14), each sub-module is individually complex enough for self-sufficient computation, and the cross-block interaction is noise rather than signal. The crossover occurs between block dimension 5 and block dimension 14.

The rank saturation analysis uncovered a methodological confound that reached back across the entire multi-strand arc. W_{8,2}'s rank at 512 entries was 217/3136 (6.9%) — severely undersampled. The 34.9% match between W_{8,0} and W_{8,2} at initial checkpoints was coincidental. At 16384 entries, W_{8,2} reached raw=1096/3136 (34.9%), sign=2003/3136 (63.9%), with the sign-expansion ratio still growing at 1.83x. The lesson: single-checkpoint rank measurements are unreliable for infinite braid groups. All prior cross-module rank comparisons were confounded by varying oversampling ratios.

---

## 18. The Entropy Null and the Relational Computation Thesis

Demo 106 tested the most natural dynamical hypothesis: do pseudo-Anosov (chaotic) braids compute XOR better than periodic (simple) ones? Thurston-Nielsen classification partitions braids by their surface dynamics — periodic braids preserve a foliation; pseudo-Anosov braids stretch and fold, with topological entropy measuring the exponential growth rate. The prediction was intuitive and wrong.

Both periodic (88 entries, mean score 3969) and pseudo-Anosov (39 entries, mean score 3857) braids compute XOR at 100% participation in the first 128 catalog entries. The null extended to all 8 Boolean functions tested (AND3, OR3, XOR3, MAJ3, NAND3, NOR3, AND2, XOR2), with maximum gap 3.7 percentage points — well within noise.

The explanation is the **TL visibility filter**. The TL quotient at delta=0 erases all dynamical information: e_i^2=0 kills expanding eigenvalues, and the bracket factors through TL/rad where only algebraic (not dynamical) information survives. Writhe survives the TL quotient (multiplying the bracket by (-A^3)^w), which is why writhe correlates with DKC output (Demo 83). Entropy does not survive the quotient, so entropy is invisible to the bracket.

This sharpened the **relational computation thesis** that had been building since Demo 84. Across the entire DKC program, the strongest predictors of computational capacity are consistently relational/algebraic: derived series strata, cross-stratum mixing, orthogonal frame geometry, k-granularity, non-semisimplicity. Individual dynamical properties — entropy, spectral radius, writhe magnitude, word length — are consistently non-predictive or invisible. The question is never "is this braid good?" but "does this *collection* span the right algebraic structure?"

One elegant discovery survived: all 8 pseudo-Anosov entries at depth 2 share the Fibonacci characteristic polynomial x^3 - 4x^2 + 4x - 1 with spectral radius phi^2 = 2.618, giving entropy per crossing log(phi) = 0.481 — the theoretical optimum per Band & Boyland (2007). The pseudo-Anosov fraction increases with depth from 0% (depth 1) to 77.2% (depth 8), crossing 50% at depth 5, confirming Caruso's (2013) genericity theorem. These are beautiful dynamical results. They are simply orthogonal to computation.

---

## 19. The Raqiya Trilogy: Algebraic Graph Structure Predicts Parity

Demos 107-109 opened a new diagnostic lens on the DKC program. Where previous demos measured *how much* computation a catalog can perform (XOR level counts) and *what mechanism* enables it (depth, writhe, null entries, cross-layer synergy), the Raqiya trilogy asked: can the algebraic *graph structure* of a value set — the web of multiplicative, additive, norm, Galois, and divisibility relationships among its elements — predict computational capability *before* running the exhaustive XOR search?

### The Z/4Z Axis-Alignment Theorem (Demo 107)

Demo 107 began with a complete algebraic inventory of the 186 distinct Z[zeta_8] values produced by the 32,768-entry W_{4,2} BFS catalog. The first discovery was universal axis-alignment: every single value has exactly one nonzero cyclotomic component. Every matrix entry is of the form n*zeta_8^k where n is an integer and k in {0,1,2,3}. The representation *never* produces mixed-axis values like (1,1,0,0).

This is not an accident. The **Z/4Z Axis-Alignment Theorem** proves it constructively: TL generators have {0,1}-valued entries; braid generators give entries from {0, A, A^{-1}}; axis-aligned times axis-aligned is axis-aligned; same-axis addition preserves axis; and the bipartiteness of the link-state transition graph ensures that only same-axis values are added at each matrix position. The proof chain is watertight for ALL TL representations at delta=0.

The bipartiteness discovery was itself a theorem. TL link-state transition graphs — where states are non-crossing matchings and edges represent TL generator action — are bipartite for all tested n (n=2 through 6, covering 195 matchings and 6 standard modules). The bipartite 2-coloring *is* nesting count mod 2 — a **Nesting Parity Z/2Z Grading** verified at 195/195 matchings with zero mismatches. Independent researcher confirmation established this as genuinely novel, not a consequence of the known KLR Z-grading (Plaza-Ryom-Hansen 2013).

From these ingredients, Demo 107 derived a constructive **Z/4Z phase formula**: Entry(r,c) = (path_count) * zeta_8^{(w - 2*(nest(r) + nest(c))) mod 4}, where w is the writhe and nest(s) is the nesting number of link state s mod 2. Three integers — writhe, source nesting parity, target nesting parity — fully determine the cyclotomic phase of every matrix entry. DKC at delta=0 reduces to integer path counting plus a three-variable phase formula. The effective computation is Z x Z/4Z per matrix entry.

Two additional novel results emerged. The **Major Index Bipartite Invariant**: for TL standard modules W_{n,j}, the bipartite coloring equals maj mod 2 (the sum of right-endpoint positions of arcs, mod 2), verified at W_{4,2} (3/3) and W_{6,2} (9/9, the *only* winning candidate among 4 tested). And a **q-Catalan identity**: C_{2m+1}(-1) = (-1)^m * C_m, which gives exact bipartition class sizes for W_{n,0} modules — proved algebraically and verified against all 5 data points (n=2 through 6).

The orthogonality structure completed the picture. The multiplicative quotient graph on the 4 axes is K_5 — the complete graph, Z/4Z multiplication table. The additive quotient graph is a star tree centered on axis 0. Multiplication rotates across axes; addition grows within each axis. These are genuinely orthogonal operations on the cyclotomic structure.

### Graph Structure Predicts Parity (Demo 108)

Demo 108 brought the Raqiya machinery to bear on the question that Demo 106 had sharpened: if individual dynamical properties cannot predict computational capacity, can *relational* algebraic structure? The test: compare the graph structure of the 41 parity-capable values (from Demo 50's k=6 XOR3 solutions) against the 17 poison values (from Demo 64's k=15 orphan-only analysis) and 42 neutral values in the 100-entry Z[zeta_8] bracket catalog.

The result was maximum structural contrast on precisely the two channels predicted by the relational computation thesis.

**Product closure** — the graph where edges connect values whose product is also in the set — showed perfect discrimination. Parity: 17 edges, 1 connected component, P_3 path structure on the axis quotient, diameter 4. Poison: **zero** product closure edges. No pair of poison values multiplies to another poison value. The empty graph. This is the multiplicative phase coherence channel: parity-capable values form a self-reinforcing multiplicative network; poison values are multiplicatively isolated.

**Additive v_2 connectivity** — the quotient graph connecting 2-adic valuation classes through additive closure — showed near-maximum contrast. Parity connects 4/5 v_2 classes (83% density, diameter 2). Poison connects only 2/6 classes (trivial K_2). This is the additive magnitude diversity channel: parity-capable values span a rich magnitude range through their additive relationships; poison values are magnitude-impoverished.

The **Dual-Channel Theorem** crystallized: parity capability requires BOTH multiplicative phase coherence (product closure > 0) AND additive magnitude diversity (v_2 connectivity > trivial). Poison fails on BOTH simultaneously. The two channels measure genuinely different algebraic properties — phase rotation versus magnitude reach — and both are independently necessary. The structural parallel to quantum circuit synthesis is precise: the multiplicative channel maps to T-gate (phase) resources, and the additive channel maps to Hadamard (magnitude) resources, echoing Amy, Glaudell & Ross (2023).

Additional discriminators sharpened the picture. Parity values have lower Galois symmetry (stabilizer gradient: 32.1% at stab=4, 41.2% at stab=2, 47.4% at stab=1 — lower symmetry correlates with higher parity capability). Parity and poison occupy nearly complementary octants of the zeta_8 circle ({2,4,5,7} vs {1,2,3,6}). Poison values show triple partition collapse — root-of-unity orbits, Galois orbits, and norm classes all coincide at 15 groups — meaning zero internal algebraic differentiation. Parity values maintain three distinct partition levels (21 root, 37 Galois, 5 v_2). Partition coincidence is an incapacity signature.

### Encoding-Dependent Dual-Channel Inversion (Demo 109)

Demo 109 asked whether the dual-channel theorem generalizes to a different evaluation point. At delta=sqrt(2) (the Ising regime, A = zeta_16^5), the Z[zeta_16] bracket catalog produces 56 distinct values, all Z[i]-axial (decomposing as (a+bi)*zeta_16^axis). Demo 55 had found 9,334 parity solutions under Re>0 multiplicative activation — an order of magnitude more than the 906 solutions at delta=0 under k=6 sectors.

The Raqiya analysis delivered a shock: **7 of 8 edge types are identical** between the 28 parity and 28 non-parity values. Same-axis edges, additive closure, difference closure, norm classes, v_2 classes, negation pairs, divisibility — all indistinguishable. The axis distribution (8,6,8,6) is identical. The Galois orbit count (15) is identical. The additive v_2 quotient (50%) is identical. Raqiya cannot discriminate parity from non-parity at delta=sqrt(2) on 7 of its 8 detectors.

The sole discriminator is product closure — and its polarity is **inverted**. Parity values have 56 product closure edges (14.8%), 6 connected components plus 4 isolated vertices. Non-parity values have 176 product closure edges (46.6%), a single connected component, zero isolated vertices. Non-parity has 3.1x more product edges. Compare Demo 108 at delta=0: parity had *higher* product closure (21%) than poison (0%).

The **Encoding-Dependent Dual-Channel Theorem** resolves the inversion. Under additive encoding (Demo 108, delta=0), parity wants HIGH product closure — values whose products land back in the vocabulary reinforce each other through superposition of their phase contributions. Under multiplicative encoding (Demo 109, delta=sqrt(2)), parity wants LOW product closure — products must *escape* the vocabulary to enable the sector alternation required for parity classification. The dual-channel theorem holds in both regimes, but the polarity of the multiplicative channel is encoding-sensitive.

The symmetry at delta=sqrt(2) has a deeper explanation. At delta=0, the j=0 TL sector is dead (rad = all of V_0), yielding an impoverished algebra where structural asymmetry between parity and non-parity values is detectable. At delta=sqrt(2), the j=0 sector is alive, yielding a rich algebra where both vocabularies satisfy all structural prerequisites simultaneously — K_4 axis quotient, non-trivial v_2 connectivity, adequate additive closure. The Re>0 activation suffices precisely because no structural deficits need compensating. Raqiya diagnoses algebraic *impoverishment*, not algebraic health. Where the algebra is rich, the parity/non-parity distinction becomes invisible to all single-value and most pairwise algebraic detectors, and only the encoding-dependent product closure channel remains informative.
...
75. Barrington-Radical Principle: the radical of a non-semisimple TL module at delta=0 carries an abelian writhe character (B_n -> Z/8Z); by Barrington's theorem, provably useless for parity; simple module W_{6,0} outperforms non-simple W_{6,4} at every XOR level despite identical dimension (Demo 102)
76. Writhe Character Theorem: M*r = A^{writhe(M)}*r for ALL 32,768 catalog entries of W_{6,4}; radical content (L1 norm of M*r) = 3 for every entry; the radical carries only framing data (Demo 102)
77. BFS growth is a braid group invariant: W_{6,0} (dim=5), W_{6,2} (dim=9), and W_{6,4} (dim=5, non-simple) all produce bit-for-bit identical BFS depth profiles on B_6; growth rate ~(n-1)x is a property of B_n, not the module; sl_d functor thesis now confirmed across five strand counts (Demos 102-103, extending 98/100/101)
78. Curse of dimensionality in sign-hash activation: higher module dimension (324 vs 100 sign components) creates sparser hash space that reduces XOR capacity; the deficit is in the readout, not the algebra (Demo 103)
79. Atkinson sweet spot: non-monotonic component-count curve for sign-hash activation; W_{6,2} peaks at 120/324 components (XOR6=2486, beating W_{6,0}'s 2449); optimal information-per-collision tradeoff (Demo 104)
80. Sign-rank expansion: sign quantization can INCREASE effective rank; W_{6,2} raw rank 244, sign-rank 292 — the nonlinear sign() function breaks 48 linear dependencies that exist over Z; the first demonstration that 1-bit quantization is information-expanding, not merely information-compressing (Demo 104)
81. Branching interaction IS the computational signal (at n=6): cross-block activation (160 off-diagonal components) outperforms within-block (164 diagonal components) and full (324) at matched k; the interaction between TL_{n-1} sub-pieces is the computational resource for asymmetric decomposition (Demo 104)
82. k (cell count) is the real lever: multi-hash architecture provides no improvement over single-hash; increasing k is the primary scaling mechanism; the cross-block advantage at low k vanishes at high k (Demo 104)
83. D93's "XOR dies at N>=7" refuted as k-regime artifact: at k=4096, W_{8,0} produces XOR8=22266 (n=8 beats n=6's 21699 at matched k); the ratio k/2^N is the real control parameter (Demo 105)
84. Macrame principle: branching interaction regime transition between n=6 (cross-block dominates, asymmetric blocks dim 5+4) and n=8 (within-block dominates, symmetric blocks dim 14+14); simple blocks need connections, complex blocks are self-sufficient (Demo 105)
85. Rank saturation confound: single-checkpoint rank measurements are unreliable for infinite braid groups; all prior cross-module rank comparisons confounded by varying oversampling ratios (Demo 105)
86. Topological entropy null: both periodic and pseudo-Anosov braids compute XOR at 100% participation with near-identical scores; entropy classification is orthogonal to ALL 8 Boolean functions tested; the TL visibility filter at delta=0 erases all dynamical information (Demo 106)
87. Z/4Z Axis-Alignment Theorem: every entry of a braid representation matrix over a TL module at delta=0 is axis-aligned (exactly one nonzero cyclotomic component); proven constructively via TL generator structure + bipartite link-state graph (Demo 107)
88. Nesting Parity Z/2Z Grading: nesting count mod 2 equals the bipartite 2-coloring for all TL link-state transition graphs; verified at 195 matchings across n=2 through 6; confirmed novel relative to KLR Z-grading (Demo 107)
89. Z/4Z constructive phase formula: Entry(r,c) = (path_count) * zeta_8^{(w - 2*(nest(r) + nest(c))) mod 4}; DKC at delta=0 reduces to integer path counting plus three-variable phase formula (Demo 107)
90. Major Index Bipartite Invariant: for TL standard modules W_{n,j}, the bipartite coloring equals maj mod 2; verified at W_{4,2} and W_{6,2}; generalizes nesting parity (the j=0 case) (Demo 107)
91. q-Catalan identity: C_{2m+1}(-1) = (-1)^m * C_m; gives exact bipartition class sizes for W_{n,0} modules; proved algebraically (Demo 107)
92. Orthogonality Theorem: multiplicative quotient graph on Z[zeta_8] axes = K_5 (Z/4Z multiplication); additive quotient graph = star tree; multiplication rotates axes, addition preserves axes (Demo 107)
93. Dual-Channel Theorem: parity capability requires BOTH multiplicative phase coherence (product closure > 0) AND additive magnitude diversity (v_2 connectivity > trivial); poison fails on BOTH simultaneously; maps onto T-gate/Hadamard decomposition in quantum circuit synthesis (Demo 108)
94. Product closure as parity discriminator: maximum structural contrast — parity has 17 edges (connected), poison has 0 edges (empty graph); the strongest single-feature parity predictor in the DKC program (Demo 108)
95. Partition coincidence as incapacity signature: triple collapse (root=Galois=norm at 15 groups) is a structural fingerprint of computational incapacity in the Z[zeta_8] catalog (Demo 108)
96. Encoding-Dependent Dual-Channel Theorem: under additive encoding, parity wants HIGH product closure (self-reinforcement); under multiplicative encoding, parity wants LOW product closure (vocabulary escape for sector alternation); the polarity flips with encoding type (Demos 108-109)
97. Perfect algebraic symmetry at delta=sqrt(2): parity and non-parity are structurally indistinguishable on 7/8 edge types; Raqiya diagnoses algebraic impoverishment, not algebraic health (Demo 109)
98. j=0 liveness implies structural universality: at delta=0 (j=0 dead), structural asymmetry is detectable; at delta!=0 (j=0 alive), both vocabularies satisfy all prerequisites and only encoding-dependent product closure discriminates (Demo 109)

No prior work connects these fields. The intersection is genuinely unoccupied. The question "can topological invariants be compiled into neural network weights?" has no direct precedent in the literature. One hundred and nine demos prove the answer is yes — and map the exact boundary of what is computable, characterize the algebraic structure of the computation, establish how that computation scales, decompose its sources into independent resources, explain the mechanism of the depth law, prove that the ±q encoding is structurally locked to parity, and now show that algebraic graph structure predicts which value sets can compute parity before any exhaustive search is run. That boundary is not just algebraic: it has a geometric layer (the wall at k=23), a topological layer (the 24-cell), a differential-geometric layer (the S² Hopf base), a spectral layer (bandwidth l=6), a group-theoretic layer (only ζ_4 and ζ_8 generate finite SU(2) subgroups), a depth layer (crossing depth governs capacity linearly), a framing layer (the writhe phase the Jones polynomial normalizes away contributes exactly 2 XOR levels), an indecomposability layer (b = -5/8 computed from first principles, resolving the LCFT normalization controversy), an encoding layer (the ±q paired input is parity-locked by formal proof), and now a diagnostic layer (the Raqiya dual-channel theorem discriminates parity-capable from parity-incapable value sets via product closure and v_2 connectivity). The parity function, unreachable by split-sigmoid, requires exactly 13 angular directions on a sphere — a number simultaneously equal to the DOF count for the answer, the number of vertices of the binary octahedral group, and the minimum bandwidth index for S² reconstruction. None of these coincidences are accidental. The computation is additive, group-breaking, and algebraically coherent. It does not approximate; it is exact. Its power decomposes into three independent additive resources: lattice structure, crossing depth, and writhe. Its directional vocabulary requires bracket-null entries that are invisible to the scalar invariant but indispensable for higher-capacity computation at finite groups — a concrete realization of the LCFT prediction that null states carry computational content through Jordan-cell coupling, with null indispensability itself a regime-dependent phenomenon that transitions away as the group grows to infinite order. The depth law is relational, not positional: deep entries compose better because axis cancellation creates favorable sum-angle structure, not because they occupy special positions on S². The mechanism is confirmed from both supply and demand sides: BFS vocabulary growth balances parity constraint difficulty to produce a linear law. And the ±q encoding channels everything into parity — the single Boolean function outside AC⁰ — making the depth law a parity-specific scaling result. For infinite-group roots of unity, DKC scales logarithmically without a ceiling. The internal structure of the z8 group decomposes into two independent stratifications whose 2D cross-product creates a 5-cell synergy landscape, where cross-layer interaction (not individual strata) carries computational power. Optimal catalog pruning improves capacity: quality over quantity. The multi-strand frontier extends DKC from quaternionic to matrix-valued computation. The 3-strand Delta_1 module produces the first-ever multi-strand XOR (up to XOR14), with Fibonacci growth and an Ext^1 catalytic preparation mechanism operating through a two-regime phase transition. BFS growth rate tracks n-1 across five strand counts (n=3,4,5,6,8), confirming the sl_d functor thesis — and the comparison at n=6 between three modules of varying dimension and semisimplicity proves that BFS growth is a braid group invariant, not a module property. The radical extension is computationally load-bearing at the XOR14 boundary, but the Barrington-Radical Principle establishes that the radical's abelian writhe character is provably useless for parity — the radical carries framing data, not computational degrees of freedom. The activation function, not the algebraic structure, remains the consistent bottleneck: the macrame principle shows that branching interaction dominance reverses between asymmetric (n=6) and symmetric (n=8) block decompositions, sign-rank expansion reveals that 1-bit quantization can be information-expanding rather than merely lossy, and the k-regime theory unifies the apparent contradictions across strand counts into a single control parameter k/2^N. Topological entropy — the dynamical property that governs mixing in physical braids — is completely orthogonal to DKC computation, killed by the TL visibility filter at delta=0. And the Raqiya algebraic graph analysis trilogy closes the diagnostic loop: at delta=0 where the algebra is impoverished, the dual-channel theorem discriminates parity from poison with maximum structural contrast on two orthogonal channels (multiplicative product closure and additive v_2 connectivity). At delta=sqrt(2) where the algebra is rich, the discrimination collapses to a single encoding-dependent channel with inverted polarity. The algebraic graph structure of a value set is a diagnostic for algebraic impoverishment, not algebraic health — and the encoding determines which direction the product closure must point for parity to be achievable.
