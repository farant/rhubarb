...
*Compiled during Demo 27 (Complex Reverse DKC). February 2026.*

## Context

Demo 27 asks whether complex-valued neural network weights trained on XOR match complex Kauffman bracket evaluations at `A = e^{i*5*pi/4}` (a primitive 8th root of unity), where the loop value `delta = -(A^2 + A^{-2}) = 0`. The demo tests whether phase information (discarded in Demo 26's amplitude-only matching) improves the correspondence, whether a U(1) gauge correction aligns weight phases, and whether the bracket values lie in the cyclotomic ring Z[zeta_8].
...
- **Relevance to Demo 27:** This is the direct theoretical justification for Part A. The demo implements exactly this architecture (CxNeuron with w1, w2, b). The 6-parameter complex neuron replaces Demo 26's 9-parameter real 2-2-1 network.
...
- **Relevance to Demo 27:** Explains why the complex neuron converges reliably on XOR and why complex matching should outperform the real network: the orthogonal structure constrains the weight space more tightly.
...
- **Authors:** Chiheb Trabelsi, Olexa Bilaniuk, et al.
- **Year:** 2018 (ICLR)
- **Key Finding:** Provides building blocks for deep complex-valued networks: complex convolutions, complex batch normalization, complex weight initialization. Demonstrates that complex networks outperform real-valued counterparts on several benchmarks.
...
- **Links:** [arXiv](https://arxiv.org/abs/1705.09792)
...
- **Relevance to Demo 27:** Supports the thesis that phase is not discardable noise. If bracket evaluations carry phase information, then a CVNN should capture it better than a real network.

### Paper 9: Lee et al. (2010) — CVNN Phase-Encoded Weights
...
- **Relevance to Demo 27:** Directly relevant to the U(1) gauge correction. If the network's global phase is a free parameter (gauge symmetry), then the meaningful information is in relative phases, and a gauge sweep should find the optimal alignment.
...
- **Year:** 2016 (ICML)
- **Key Finding:** Constraining the hidden-to-hidden weight matrix to be unitary (eigenvalues on the unit circle, |lambda|=1) solves vanishing/exploding gradient problems. The unitary constraint is parameterized by compositions of structured matrices (diagonal, reflection, permutation, FFT).
- **Relevance to Demo 27:** The unitary constraint forces weights onto a structure where phases matter intrinsically. If unconstrained training produces weights near cyclotomic integers (which have controlled norms), this suggests a connection between gradient geometry and algebraic number theory.
...
- **Relevance to Demo 27:** This is the mathematical object the demo computes. The code in `braid_bracket_at()` implements exactly this state sum. At A = e^{i*5*pi/4}, delta = 0, which kills all multi-loop states and produces a dramatic simplification.
...
- **Key Finding:** An explicit polynomial quantum algorithm approximates the Jones polynomial at any primitive root of unity. The problem is BQP-complete.
- **Relevance to Demo 27:** The BQP-completeness means classically computing the Jones polynomial at roots of unity is believed intractable in general. Demo 27's approach of enumerating small braids sidesteps this by working with short braid words. The fact that a neural network "discovers" bracket values through gradient descent is remarkable precisely because the underlying computation is quantum-hard in general.
- **Links:** [arXiv](https://arxiv.org/abs/quant-ph/0511096)
...
- **Key Finding:** Proved the equivalence between TQFT and quantum computation. Approximating the Jones polynomial at the 5th root of unity is BQP-complete, extended to k >= 7.
- **Relevance to Demo 27:** Establishes that the Kauffman bracket at roots of unity is the canonical BQP-complete problem. Demo 27 evaluates at the 8th root (k=8), in the BQP-hard regime.
- **Links:** [arXiv](https://arxiv.org/abs/quant-ph/0101025)
...
- **Key Finding:** Multiplicative approximation of the Jones polynomial is #P-hard. Additive approximation at non-lattice roots of unity is BQP-complete. Sharp complexity-theoretic boundary.
- **Relevance to Demo 27:** Demo 27's "reverse" approach (training a classical neural network to match bracket values) is performing a task in an interesting complexity-theoretic regime.
...
### Paper 15: Bonahon, Wong (2016) — Representations of Skein Algebra
...
- **Relevance to Demo 27:** At the 8th root of unity, the skein algebra has special algebraic properties. The center determines the representation theory, meaning bracket values at these special A values have additional structure.
- **Links:** [arXiv](https://arxiv.org/abs/1206.1638)
...
- **Year:** 2019
- **Journal:** Inventiones Mathematicae
- **Key Finding:** Resolves the unicity conjecture: irreducible representations are generically classified by their central characters. The skein algebra is finitely generated as a module over its center.
- **Relevance to Demo 27:** Finite generation over the center means finitely many "types" of bracket values at any root of unity. Consistent with Demo 27's finding that bracket values cluster in a discrete set (Z[zeta_8]).
- **Links:** [arXiv](https://arxiv.org/abs/1707.09234)

---

## 7. Cyclotomic Integers and the Ring Z[zeta_8]

### Paper 17: Structure of Q(zeta_8) (Algebraic Number Theory)

- **Standard algebraic number theory result**
- **Key Finding:** Q(zeta_8) = Q(i, sqrt(2)) is a degree-4 extension of Q with integral basis {1, zeta_8, zeta_8^2, zeta_8^3}. The ring Z[zeta_8] is a Unique Factorization Domain (class number 1).
- **Relevance to Demo 27:** This is exactly what Part B's "cyclotomic lattice test" checks: whether bracket values have coordinates that are multiples of 1/sqrt(2). Algebraically well-motivated since bracket values (Z-polynomial expressions in A and A^{-1}) necessarily lie in Z[zeta_8].

---

## 8. Witten-Chern-Simons Theory (Physical Context)

### Paper 18: Witten (1989) — QFT and the Jones Polynomial
...
- **Key Finding:** The Jones polynomial arises as the expectation value of Wilson loop operators in SU(2) Chern-Simons gauge theory at level k. The variable in the Jones polynomial is q = e^{2*pi*i/(k+2)}.
- **Relevance to Demo 27:** Provides the physical interpretation: bracket values are not arbitrary complex numbers — they are transition amplitudes in a topological quantum field theory.
...
### Paper 19: Hughes (2016) — NN Knot Invariant Prediction

- **Title:** "A Neural Network Approach to Predicting and Computing Knot Invariants"
- **Authors:** Mark C. Hughes
- **Year:** 2016
- **Key Finding:** A two-layer feed-forward neural network can predict the s-invariant from the Khovanov polynomial with >99% accuracy. Neural networks can predict quasipositivity of braid representations.
- **Relevance to Demo 27:** This is the "forward" direction (knot -> invariant), whereas Demo 27 is the "reverse" direction (trained weights -> bracket values).
...
- **Year:** 2021
- **Journal:** Nature
- **Key Finding:** Machine learning revealed that the knot signature is directly related to hyperbolic geometry (the "natural slope"). Attribution techniques in ML guided the discovery of a new theorem.
- **Relevance to Demo 27:** Demonstrates the "ML as mathematical conjecture generator" paradigm. Demo 27 is in this spirit.
- **Links:** [Nature](https://www.nature.com/articles/s41586-021-04086-x)
...
- **Key Finding:** Survey of techniques for obtaining zero-error results with ML in math and physics.
- **Relevance to Demo 27:** Methodological context for ML-guided mathematical discovery.
...
- **Year:** 2025
- **Key Finding:** Braid representations outperform other encodings for ML prediction of topological invariants.
- **Relevance to Demo 27:** Validates Demo 27's use of braid words as the input representation.
...
- **Year:** 2019-2021
- **Key Finding:** Permutation symmetry of neurons creates equivalent global minima connected by equal-loss valleys. The symmetry group of a network's weight space determines the topology of the loss landscape.
- **Relevance to Demo 27:** Part C's U(1) gauge sweep is a continuous analogue of this discrete permutation symmetry. In a complex network, multiplying all weights by a global phase e^{i*theta} changes nothing functionally — the gauge correction finds the representative closest to the bracket lattice.
...
- **Key Finding:** Quantum neural networks can be mapped onto spin-networks, analyzed through TQFT. The transition amplitude (output) is a topological invariant, giving the network inherent fault tolerance.
- **Relevance to Demo 27:** If bracket values ARE topological invariants, and NN weights converge to bracket values, then the network is encoding topological invariants in its weight structure.
...
- **Key Finding:** Constructs a functor from cone-cocone diagrams to cobordisms of Hilbert spaces. Machine learning with functorial evolution governed by TQFT amplitudes.
- **Relevance to Demo 27:** Provides categorical framework where NN computation and TQFT share formal structure.
- **Links:** [Wiley](https://onlinelibrary.wiley.com/doi/abs/10.1002/prop.202200104)

---

## 12. Abramsky — Temperley-Lieb as Computational Bridge
...
- **Relevance to Demo 27:** The TL algebra is the algebraic structure underlying the AJL algorithm. The bracket state sum is a trace in the TL algebra. If NN weights converge to TL-algebraic values, this connects NN optimization to TL representation theory.
...
### Paper 27: Recent (2025) — Classical Braiding in Metamaterials
...
- **Relevance to Demo 27:** Demonstrates that topological computation is not exclusively quantum. A classical system matching bracket values would be another instance of classical systems capturing topological quantum structure.
...
- **No prior work** asks whether gradient-trained complex neural network weights are elements of Z[zeta_8].
...
- **No prior work** studies the specific value A = e^{i*5*pi/4} where delta=0.

Demo 27's thesis lives at an unoccupied intersection of these established fields.
